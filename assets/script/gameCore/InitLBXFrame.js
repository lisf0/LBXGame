...
cc.Class({
    ...
    onLoad: function () {
        ...
        var posList = [
            ...
            {
                count: 6,
                srcPos: cc.v2(2 * this["liubianxingH"], 0)
            },

            //第三行的位置信息
            {
                count: 7,
                srcPos: cc.v2(2 * this["liubianxingH"] * 2, 0)
            },

            //第四行的位置信息
            {
                count: 8,
                srcPos: cc.v2(2 * this["liubianxingH"] * 3, 0)
            },

            //第五行的位置信息
            {
                count: 9,
                srcPos: cc.v2(2 * this["liubianxingH"] * 4, 0)
            },

            //第六行的位置信息
            {
                count: 8,
                srcPos: cc.v2(2 * this["liubianxingH"] * 4 + this["liubianxingH"], (-3 * this["liubianxingA"]) / 2)
            },

            //第七行的位置信息
            {
                count: 7,
                srcPos: cc.v2(2 * this["liubianxingH"] * 4 + this["liubianxingH"] * 2, (-3 * this["liubianxingA"] * 2) / 2)
            },

            //第八行的位置信息
            {
                count: 6,
                srcPos: cc.v2(2 * this["liubianxingH"] * 4 + this["liubianxingH"] * 3, (-3 * this["liubianxingA"] * 3) / 2)
            },

            //第九行的位置信息
            {
                count: 5,
                srcPos: cc.v2(2 * this["liubianxingH"] * 4 + this["liubianxingH"] * 4, (-3 * this["liubianxingA"] * 4) / 2)
            },
        ...
        //要加的单位向量
        var addVec = pForAngle(240 * (2 * Math.PI / 360)).mul(this["liubianxingH"] * 2);

        //偏移至源点0，0的向量
        var pianyiTo0p0Vec = pForAngle(120 * (2 * Math.PI / 360)).mul(this["liubianxingH"] * 2 * 4);
        ...
        for (var i = 0; i < posList.length; i++) {
            ...
            var oneSrcPos = posList[i].srcPos.add(pianyiTo0p0Vec)//起始位置
            var aimPos = srcPos.add(oneSrcPos)//一条的起始位置

            for (var j = 0; j < count; j++) {
                var fPos = aimPos.add(addVec.mul(j))
                fPosList.push(fPos)
            }
        }
